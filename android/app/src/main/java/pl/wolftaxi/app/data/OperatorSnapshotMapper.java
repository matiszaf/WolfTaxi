...
public final class OperatorSnapshotMapper {
    ...
    public static OperatorSnapshot fromJson(JSONObject json) {
        ...
        if (drivers != null) for (int i=0;i<drivers.length();i++) {
            ...
            d.queueRegionId=j.optString("queueRegionId",""); d.queuePosition=j.optInt("queuePosition",0); d.queueSize=j.optInt("queueSize",0);
        ...
        JSONArray messages=json.optJSONArray("messages");if(messages!=null)for(int i=0;i<messages.length();i++){JSONObject j=messages.optJSONObject(i);if(j==null)continue;DispatchMessage m=new DispatchMessage();m.id=j.optString("id","");m.type=j.optString("type","info");m.title=j.optString("title","");m.body=j.optString("body","");m.requiresAck=j.optBoolean("requiresAck",false);m.createdAt=j.optLong("createdAt",0);out.messages.add(m);}
    ...
    private static Order order(JSONObject json) {
        ...
        out.passengerCount=json.optInt("passengerCount",1); out.cardRequired=json.optBoolean("cardRequired",false); out.estimatedPrice=json.optDouble("estimatedPrice",0); out.finalPrice=json.optDouble("finalPrice",0);
