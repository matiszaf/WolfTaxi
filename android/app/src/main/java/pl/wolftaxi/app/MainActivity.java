...
public final class MainActivity extends Activity implements BackendListener, OperatorListener {
    ...
    @Override public void onDestroy() { backend.stop(); operatorBackend.stop(); handler.removeCallbacksAndMessages(null); super.onDestroy(); }
    ...
    private void dashboardShell() {
        ScrollView scroll=baseScroll(); root=Ui.column(this); root.setPadding(Ui.dp(this,14),Ui.dp(this,14),Ui.dp(this,14),Ui.dp(this,28)); scroll.addView(root); setContentView(scroll);
    }
    ...
    private LinearLayout compactCard(LinearLayout row){LinearLayout card=Ui.column(this);card.setPadding(Ui.dp(this,10),Ui.dp(this,10),Ui.dp(this,10),Ui.dp(this,10));android.graphics.drawable.GradientDrawable bg=new android.graphics.drawable.GradientDrawable();bg.setColor(Ui.CARD);bg.setCornerRadius(Ui.dp(this,12));bg.setStroke(Ui.dp(this,1),Color.rgb(33,61,69));card.setBackground(bg);LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1f);p.setMargins(Ui.dp(this,3),0,Ui.dp(this,3),Ui.dp(this,10));row.addView(card,p);return card;}
    ...
    private void renderOffer(Order order){LinearLayout card=Ui.card(this,root);Ui.text(this,card,"NOWE ZLECENIE",14,Ui.ORANGE,true);countdown=Ui.text(this,card,"",34,Ui.ORANGE,true);Ui.text(this,card,safe(order.pickupAddress),24,Ui.TEXT,true);if(!safe(order.destinationAddress).isEmpty())Ui.text(this,card,"→ "+order.destinationAddress,18,Ui.TEXT,false);String meta=order.passengerCount+" os. · "+order.paymentMethod.label;if(order.cardRequired)meta+=" · karta wymagana";Ui.text(this,card,meta,13,Ui.MUTED,false);if(!safe(order.notes).isEmpty())Ui.text(this,card,order.notes,13,Ui.MUTED,false);if(order.estimatedPrice>0)Ui.text(this,card,"Szacunkowo: "+money.format(order.estimatedPrice),14,Ui.TEXT,true);LinearLayout row=Ui.row(this);card.addView(row);Ui.rowButton(this,row,"ODRZUĆ",Ui.RED,v->confirm("Odrzucić zlecenie?",()->action(cb->backend.rejectOrder(order.id,cb))));Ui.rowButton(this,row,"PRZYJMIJ",Ui.GREEN,v->action(cb->backend.acceptOrder(order.id,cb)));startCountdown(order.id,order.offerExpiresAt);}
    private void renderActiveOrder(Order order){LinearLayout card=Ui.card(this,root);Ui.text(this,card,"ZLECENIE "+safe(order.id),13,Ui.BLUE,true);Ui.text(this,card,order.status.label.toUpperCase(Locale.ROOT),24,Ui.TEXT,true);Ui.text(this,card,"PODSTAWIENIE",11,Ui.MUTED,true);Ui.text(this,card,safe(order.pickupAddress),21,Ui.TEXT,true);if(!safe(order.destinationAddress).isEmpty()){Ui.text(this,card,"CEL",11,Ui.MUTED,true);Ui.text(this,card,order.destinationAddress,19,Ui.TEXT,false);}if(!safe(order.passengerPhone).isEmpty())Ui.text(this,card,"Kontakt: "+order.passengerPhone,13,Ui.MUTED,false);if(!safe(order.notes).isEmpty())Ui.text(this,card,"Uwagi: "+order.notes,13,Ui.MUTED,false);LinearLayout utilities=Ui.row(this);card.addView(utilities);String navAddress=order.status==OrderStatus.IN_PROGRESS&&!safe(order.destinationAddress).isEmpty()?order.destinationAddress:order.pickupAddress;Ui.rowButton(this,utilities,"NAWIGACJA",Ui.BLUE,v->openNavigation(navAddress));if(!safe(order.passengerPhone).isEmpty())Ui.rowButton(this,utilities,"ZADZWOŃ",Ui.BLUE,v->openDialer(order.passengerPhone));OrderStatus next=nextStatus(order.status);if(next!=null)Ui.button(this,card,nextAction(next),next==OrderStatus.COMPLETED?Ui.RED:Ui.GREEN,v->nextOrderAction(order,next));}
    ...
    private void renderMessages(){LinearLayout card=Ui.card(this,root);Ui.text(this,card,"CENTRALA",13,Ui.MUTED,true);if(snapshot.messages.isEmpty()){Ui.text(this,card,"Brak wiadomości.",13,Ui.MUTED,false);return;}for(int i=0;i<Math.min(snapshot.messages.size(),3);i++){DispatchMessage m=snapshot.messages.get(i);int color="warning".equals(m.type)||"urgent".equals(m.type)?Ui.ORANGE:Ui.TEXT;String time=m.createdAt>0?clock.format(new Date(m.createdAt))+" · ":"";Ui.text(this,card,time+safe(m.title),12,Ui.MUTED,true);Ui.text(this,card,safe(m.body),14,color,false);}}
    ...
    private void renderDriverActions(){LinearLayout primary=Ui.row(this);root.addView(primary);if(!snapshot.driver.onShift)Ui.rowButton(this,primary,"ROZPOCZNIJ ZMIANĘ",Ui.GREEN,v->action(cb->backend.startShift((ok,msg)->{cb.complete(ok,msg);if(ok)runOnUiThread(()->ensureLocationService(true));})));else{Ui.rowButton(this,primary,"STATUS",Ui.BLUE,v->chooseStatus());Ui.rowButton(this,primary,"REGION",Ui.BLUE,v->chooseRegion());Ui.rowButton(this,primary,"TARYFA",Ui.BLUE,v->chooseTariff());}if(snapshot.driver.onShift&&snapshot.queuePosition>0)Ui.button(this,root,"OPUŚĆ KOLEJKĘ",Ui.ORANGE,v->action(backend::leaveQueue));if(snapshot.driver.onShift)Ui.button(this,root,"ZAKOŃCZ ZMIANĘ",Color.rgb(93,112,118),v->confirm("Zakończyć zmianę?",()->action(cb->backend.endShift((ok,msg)->{cb.complete(ok,msg);if(ok)runOnUiThread(this::stopLocationService);})))) ; renderSessionActions();}
    ...
    private void renderOperatorDashboard(){
        ...
        renderOperatorStats(); renderOperatorActions(); renderOperatorDrivers(); renderOperatorOrders(); renderOperatorMessages(); if(mode==AppMode.ADMIN)renderAdminSummary(); renderSessionActions(); toastLine=Ui.text(this,root,"",13,Ui.MUTED,false);
    }
    ...
    private void newOrderDialog(){if(operatorSnapshot==null)return;LinearLayout box=dialogColumn();EditText pickup=plainField("Adres podstawienia");EditText destination=plainField("Adres docelowy");box.addView(pickup);box.addView(destination);Spinner regions=spinner(labelsRegions());Spinner tariffs=spinner(labelsTariffs());box.addView(regions);box.addView(tariffs);new AlertDialog.Builder(this).setTitle("Nowe zlecenie").setView(box).setNegativeButton("Wróć",null).setPositiveButton("UTWÓRZ",(d,w)->{String region=regions.getSelectedItemPosition()<=0?"":operatorSnapshot.regions.get(regions.getSelectedItemPosition()-1).id;String tariff=tariffs.getSelectedItemPosition()<=0?"":operatorSnapshot.tariffs.get(tariffs.getSelectedItemPosition()-1).id;operatorAction(cb->operatorBackend.createOrder(pickup.getText().toString(),destination.getText().toString(),region,tariff,cb));}).show();}
    ...
    private void messageDialog(){LinearLayout box=dialogColumn();EditText title=plainField("Tytuł");EditText body=plainField("Treść");body.setSingleLine(false);body.setMinLines(3);box.addView(title);box.addView(body);new AlertDialog.Builder(this).setTitle("Komunikat do kierowców").setView(box).setNegativeButton("Wróć",null).setPositiveButton("WYŚLIJ",(d,w)->operatorAction(cb->operatorBackend.sendMessage(title.getText().toString(),body.getText().toString(),"info",cb))).show();}
