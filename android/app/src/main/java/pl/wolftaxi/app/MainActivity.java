...
public final class MainActivity extends Activity implements BackendListener, OperatorListener {
    ...
    private void renderActiveOrder(Order order){
        ...
        if(order.meterActive || order.meterAmount>0 || order.status==OrderStatus.IN_PROGRESS){
            ...
            Ui.text(this,meter,meterMeta,11,order.meterWaitingActive?Ui.ORANGE:Ui.MUTED,false);
        }
        if(order.status==OrderStatus.IN_PROGRESS){
            String waitingLabel=order.meterWaitingActive
                ? "WZNÓW JAZDĘ · POSTÓJ " + Math.max(0,(int)Math.floor(order.meterWaitingSeconds/60.0)) + " MIN"
                : "POSTÓJ";
            Ui.button(this,card,waitingLabel,order.meterWaitingActive?Ui.GREEN:Ui.ORANGE,
                v->action(cb->backend.setMeterWaiting(order.id,!order.meterWaitingActive,cb)));
        }
