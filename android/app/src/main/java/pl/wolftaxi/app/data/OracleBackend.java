...
public final class OracleBackend implements Backend {
    ...
    @Override public void setStatusForRegion(DriverStatus status, String regionId, ActionCallback callback) {
        JSONObject payload = body("status", status.wire);
        try { payload.put("regionId", regionId == null ? "" : regionId); } catch (Exception ignored) {}
    ...
    @Override public void setMeterWaiting(String orderId, boolean active, ActionCallback callback) {
        JSONObject payload = body("active", active);
        action("/api/v1/orders/" + orderId + "/meter/waiting", payload, callback);
    }
