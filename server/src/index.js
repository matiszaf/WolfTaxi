...
app.get('/health', asyncRoute(async (_req, res) => {
  ...
  res.json({ ok: true, service: 'WolfTaxi Oracle API', version: '0.9.1', time: new Date().toISOString() });
}));
...
app.get('/api/v1/public/track/:token', trackingLimiter, asyncRoute(async (req, res) => {
  ...
  const row = (await pool.query(`
    SELECT o.id,o.status,o.pickup_address,o.destination_address,o.tracking_enabled,o.tracking_expires_at,
           o.meter_amount,o.meter_distance_m,o.meter_waiting_seconds,o.meter_waiting_active,o.meter_last_lat,o.meter_last_lng,o.meter_updated_at,
  ...
  res.json({
    ...
    meterAmount:Number(row.meter_amount||0), meterDistanceM:Number(row.meter_distance_m||0), meterWaitingSeconds:Number(row.meter_waiting_seconds||0), meterWaitingActive:!!row.meter_waiting_active
  });
}));
...
app.post('/api/v1/orders/:id/meter/waiting', ...driverGuard, asyncRoute(async (req, res) => {
  const active = req.body?.active === true;
  const result = await tx(async client => {
    const row = (await client.query(`SELECT o.*,t.start_fee,t.price_per_km,t.waiting_price_per_hour,COALESCE(z.multiplier,1) multiplier
      ...
      WHERE o.id=$1 AND o.assigned_driver_id=$2 FOR UPDATE OF o`, [req.params.id, req.driverId])).rows[0];
    if (!row) throw statusError(404, 'Brak aktywnego zlecenia.');
    if (row.status !== 'in_progress' || !row.meter_active) throw statusError(409, 'Taksometr nie jest aktywny.');
    let waiting = Math.max(0, Number(row.meter_waiting_seconds || 0));
    if (row.meter_waiting_active && row.meter_last_at) {
      waiting += Math.max(0, Math.min(120, (Date.now() - new Date(row.meter_last_at).getTime()) / 1000));
    }
    const distance = Math.max(0, Number(row.meter_distance_m || 0));
    const billedDistance = Math.floor(distance / 50) * 50;
    const multiplier = Math.max(0.01, Number(row.multiplier || 1));
    const amount = (Number(row.start_fee || 0) + (billedDistance/1000)*Number(row.price_per_km || 0) + (waiting/3600)*Number(row.waiting_price_per_hour || 0)) * multiplier;
    await client.query(`UPDATE orders SET meter_waiting_active=$2,meter_waiting_seconds=$3,meter_amount=$4,meter_last_at=now(),meter_updated_at=now() WHERE id=$1`, [row.id, active, waiting, amount]);
    await orderEvent(client, row.id, req.userId, req.driverId, active ? 'meter.waiting.start' : 'meter.waiting.stop', {});
    return {active, amount, waiting};
  });
  realtime.broadcastOperators('refresh',{reason:'meter.waiting',orderId:req.params.id,active});
  realtime.broadcastDrivers('refresh',{reason:'meter.waiting',orderId:req.params.id});
  res.json({ok:true,message:active?'Postój zgłoszony.':'Wznowiono jazdę.',meterWaitingActive:result.active,meterAmount:result.amount,meterWaitingSeconds:result.waiting});
}));
...
app.post('/api/v1/orders/:id/advance', ...driverGuard, asyncRoute(async (req, res) => {
  ...
  await tx(async client => {
    ...
    if (next === 'completed') {
      ...
      const fare = (await client.query(`SELECT t.minimum_fare,COALESCE(z.multiplier,1) multiplier FROM drivers d LEFT JOIN tariffs t ON t.id=COALESCE(NULLIF($2,''),d.current_tariff_id) LEFT JOIN fare_zones z ON z.id=d.current_fare_zone_id WHERE d.id=$1`, [req.driverId, order.tariff_id])).rows[0] || {};
      const minimumFare = Math.max(0, Number(fare.minimum_fare || 0)) * Math.max(0.01, Number(fare.multiplier || 1));
      const finalPrice = Math.max(Number.isFinite(requestedFinal) && requestedFinal > 0 ? requestedFinal : 0, meterPrice, minimumFare, Number(order.final_price || 0));
      const paymentMethod = String(req.body?.paymentMethod || order.payment_method || 'cash');
      await client.query("UPDATE orders SET status='completed',final_price=$2,payment_method=$3,completed_at=now(),meter_active=false,meter_waiting_active=false,meter_updated_at=now(),tracking_expires_at=now()+interval '24 hours',updated_at=now() WHERE id=$1", [order.id, finalPrice, paymentMethod]);
...
async function startMeter(client, orderId, driverId){
  const x=(await client.query(`SELECT o.id,o.tariff_id,d.last_lat,d.last_lng,t.start_fee,
  ...
  const initial=Number(x.start_fee||0)*multiplier;
  await client.query(`UPDATE orders SET meter_active=true,meter_waiting_active=false,meter_started_at=now(),meter_last_at=now(),meter_last_lat=$2,meter_last_lng=$3,
...
async function updateMeterForLocation(client,driverId,lat,lng,speed,accuracy){
  const x=(await client.query(`SELECT o.id,o.meter_last_at,o.meter_last_lat,o.meter_last_lng,o.meter_distance_m,o.meter_waiting_seconds,o.meter_waiting_active,
      t.start_fee,t.price_per_km,t.waiting_price_per_hour,COALESCE(z.multiplier,1) multiplier
    FROM orders o JOIN drivers d ON d.id=o.assigned_driver_id
    LEFT JOIN tariffs t ON t.id=COALESCE(o.tariff_id,d.current_tariff_id) LEFT JOIN fare_zones z ON z.id=d.current_fare_zone_id
  ...
  if(x.meter_waiting_active){
    waiting+=dt;
  } else if(x.meter_last_lat!=null&&x.meter_last_lng!=null&&Number(accuracy||0)<=100&&dt>0){
    const step=haversineMeters(Number(x.meter_last_lat),Number(x.meter_last_lng),lat,lng);
    const maxStep=Math.max(200,dt*70);
    if(Number.isFinite(step)&&step<=maxStep) distance+=step;
  }
  const billedDistance=Math.floor(distance/50)*50;
  const multiplier=Math.max(0.01,Number(x.multiplier||1));
  const base=Number(x.start_fee||0)+(billedDistance/1000)*Number(x.price_per_km||0)+(waiting/3600)*Number(x.waiting_price_per_hour||0);
  ...
  return { orderId:String(x.id), meterAmount:amount, meterDistanceM:distance, meterWaitingSeconds:waiting, meterWaitingActive:!!x.meter_waiting_active, meterUpdatedAt:Date.now() };
}

const port = Math.max(1, Number(process.env.PORT || 8081));
const host = process.env.HOST || '127.0.0.1';
const server = app.listen(port, host, () => console.log(`[WolfTaxi] API 0.9.1 TAXIMETER 50M + MANUAL WAITING działa na http://${host}:${port}`));
