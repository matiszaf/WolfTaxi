...
async function getOperatorSnapshot(client, includeUsers = false) {
  const [drivers, orders, regions, tariffs, zones, messages, users] = await Promise.all([
    client.query(`
      SELECT d.id,d.taxi_id,d.number,d.name,d.vehicle_id,d.enabled,d.on_shift,d.status,d.online,
             d.current_region_id,d.detected_region_id,d.current_tariff_id,d.current_fare_zone_id,d.active_order_id,
             d.last_lat,d.last_lng,d.last_speed,d.last_heading,d.last_accuracy,d.last_location_at,
             q.region_id AS queue_region_id,
             CASE WHEN q.driver_id IS NULL THEN 0 ELSE
               (SELECT count(*)::int FROM queue_entries q2 WHERE q2.region_id=q.region_id AND q2.joined_at<=q.joined_at)
    ...
    `),
    client.query(`SELECT * FROM orders WHERE status <> 'completed' OR completed_at > now() - interval '12 hours' ORDER BY created_at DESC LIMIT 100`),
    ...
    client.query(`SELECT id,type,title,body,requires_ack,active,created_at FROM messages WHERE active=true ORDER BY created_at DESC LIMIT 30`),
  ...
  return {
    drivers: drivers.rows.map(d => ({
      id:text(d.id), taxiId:text(d.taxi_id), number:d.number || 0, name:text(d.name), vehicleId:text(d.vehicle_id),
    ...
    messages: messages.rows.map(m => ({ id:text(m.id), type:text(m.type), title:text(m.title), body:text(m.body), requiresAck:!!m.requires_ack, active:!!m.active, createdAt:ms(m.created_at) })),
