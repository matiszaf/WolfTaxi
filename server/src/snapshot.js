...
function mapDriver(row) {
  return {
    ...
    activeOrderId: text(row.active_order_id)
  };
}
...
function mapOrder(row) {
  ...
  return {
    ...
    paymentMethod: text(row.payment_method || 'cash')
  };
}
...
async function getSnapshot(client, driverId) {
  ...
  const [regions, tariffs, zones, active, offer, history, messages, queue] = await Promise.all([
    ...
    client.query("SELECT * FROM orders WHERE assigned_driver_id=$1 AND status='completed' ORDER BY completed_at DESC NULLS LAST, updated_at DESC LIMIT 10", [driverId]),
    client.query("SELECT id,type,title,body,created_at,requires_ack FROM messages WHERE active=true ORDER BY created_at DESC LIMIT 20"),
    client.query(`
      SELECT x.position, x.total
      FROM (
        SELECT driver_id,
               row_number() OVER (ORDER BY joined_at ASC)::int AS position,
  ...
  return {
    ...
    messages: messages.rows.map(m => ({ id:text(m.id), type:text(m.type), title:text(m.title), body:text(m.body), createdAt:ms(m.created_at), requiresAck:!!m.requires_ack })),
